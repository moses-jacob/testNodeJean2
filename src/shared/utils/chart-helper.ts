...
export function createBarChartConfiguration(
  labels: string[] = [],
  dataValues: number[] = [],
  chartTitle = '',
): ID3BarChartConfig {
...
export async function generateBarChartBuffer(
  config: ReturnType<typeof createBarChartConfiguration>,
): Promise<Buffer> {
...
export function getTopIssues(actions: Action[]): IssueSummary[] {
  ...
  issues = issues.concat
    ...
    .filter(
      (value: IssueSummary, index, self) =>
        index === self.findIndex((t: IssueSummary) => t.name === value.name),
    )
    .sort((a, b) => (a.name > b.name ? 1 : b.name > a.name ? -1 : 0));

  for (const issue of issues) {
    const issueActions = actions.filter((x) =>
      x.issues?.find((y) => y.name === issue.name),
    );
...
export function getTopCities(actions: Action[]): CitySummary[] {
